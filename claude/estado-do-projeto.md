...
| Sessão | O que saiu |
|---|---|
| **17/09 (a Reserva nasce)** | **A terceira tela em espera saiu do papel**, e com ela a outra metade da aposta: *"quanto eu tenho"* · nasce **`PUT /contas/{id}/valor-atual`**, e o ponto dele é que **não existe campo de valor para sobrescrever**: o sistema grava a **diferença** como lançamento de rendimento, com data, no Extrato e no histórico · **`ValorInformado`** carrega os 30 dias, e a decisão nova foi a borda: *aplicação que nunca teve valor informado **não** é desatualizada* — marcar de velho o que nunca existiu é ruído · o `PUT` é idempotente no que importa: informar o mesmo valor devolve 200 e **não grava nada** · **`V008`**, porque o `CHECK` de `evento.tipo` é lista fechada e enum novo sem migration só quebra na hora em que alguém usa — o `V006` já avisava disso no próprio arquivo · o tipo **já estava reservado no `evento.md` como `VALOR_DE_APLICACAO_INFORMADO`**, e eu tinha criado `VALOR_DA_`: o doc ganhou a regra e eu troquei o código, não o contrário · achado do navegador: criei o evento e **esqueci a frase dele no Diário**, que mostrava o enum cru · **147 testes de unidade e 87 de integração** |
| **17/09 (o Extrato ganha totais)** | Achado do Abner usando a tela: o filtro de conta do Extrato mostrava as linhas e **nenhum total** — para saber o saldo do Nubank era filtrar e somar à mão, numa tela cujo cabeçalho diz *"saldo é sempre a soma disto"*. `GET /contas` ganha **`previstoAteOFimDoMesCentavos`** e **`saldoProjetadoCentavos`**, e a faixa mostra os **três** números: agora, previsto e projetado — o do meio é a conta que produz o terceiro, que é a regra do `dashboard.md` sobre projeção sem a conta à vista · **o `conta.md` dizia que saldo projetado não existia**, e a Home já projetava desde 17/09: linha corrigida, e o que falta de verdade — descontar o `a pagar` das faturas — foi renomeado para o que é · nasce **`JanelaDoPrevisto`** no domínio, porque o horizonte (*de hoje até o fim do mês*) estava escrito dentro do `ResumirMesUseCase` e eu ia repeti-lo: dois lugares mostrando o mesmo número não podem calculá-lo duas vezes · **com `TODAS AS CONTAS` não há números**, e a ausência é a resposta: somar corrente com aplicação não responde pergunta nenhuma, e as três leituras que respondem já estão na Home |
| **17/09 (a abertura se corrige)** | Achado do Abner ao usar a tela: o saldo de abertura **não tinha como ser corrigido**. Os dois docs diziam *"o saldo de abertura se corrige editando o valor"* desde 01/09, e o `PATCH` recusava **todo** lançamento do ciclo — `PATCH /contas` também não aceita `saldoInicial`. Regra escrita, implementada em lugar nenhum: o mesmo padrão do `listarDoAlvo` e do `exigirDaConta`. Agora `corrigido` aceita **só o `valor`** quando `doCiclo`, e recusa o resto — **excluir continua recusado**, e as duas coisas não se contradizem. A tela ganha `CORRIGIR O VALOR`, com um campo só. **Guarda anotada no doc:** a rolagem é par que soma zero, e quando nascer não pode entrar nessa regra sem guarda própria |
| **17/09 (o cache misturava versões)** | Bug do Abner, e o mais instrutivo da sessão: o **"+" de lançar** e o **"Editar"** do detalhe morreram, e **só a transferência continuava editando**. Não era dado — a conta dele foi replicada inteira num usuário de teste, meios com nome legado e tudo, e funcionou. Era o **navegador**: o front é servido sem `Cache-Control`, e aí o navegador guarda cada arquivo por **heurística** (~10% da idade dele). O `contas.js`, parado havia dias, ficou em cache; o `detalhe.js` e o `lancar.js`, mexidos na mesma hora, foram buscados novos — e os dois novos chamavam uma função que o velho não tinha. A transferência escapava porque é o único caminho que não monta seletor de meio. **Lição que vale além deste bug:** criar dependência de um arquivo quente para um arquivo frio é seguro em qualquer projeto com hash no nome, e é uma bomba-relógio neste, que não tem. `no-cache` no `application.yml` e **`FrontEstaticoIT`** para não voltar |
| **17/09 (o lançamento se corrige)** | A lacuna *"o detalhe não edita campo nenhum"* fechou: os três primeiros blocos do modal **viram formulário** no lugar em que estavam · o `PATCH` ganha **`meioId`**, que faltava — e com ele a regra que o `POST` já praticava: **quem manda na conta é o meio**, então "trocar de banco" e "trocar de meio" são um campo só, e conta divergente do meio é **recusada** em vez de gravada · `Meio.exigirDaConta` **existia desde 16/09 sem um único chamador**, e é ela que recusa · a **`dataEfeito` virou consequência, não campo livre**: corrigir a data de um Pix move as duas, e trocar boleto por Pix junta as duas no dia do evento — o campo de vencimento some da tela junto · nasce `corrigidoNaTransferencia`, porque mover dinheiro entre contas não tem vencimento e a `dataEvento` move as duas datas dos dois lados · o **impacto é contínuo**, não um aviso no fim: enquanto os campos mudam, a linha diz em quanto o saldo de **cada** conta envolvida fica, e são duas contas quando o meio leva o lançamento embora · o rótulo `conta · meio` estava escrito em **três** lugares e virou `Contas.identidadeDoMeio` · **80 testes de integração**, e o navegador dirigido em 16 passos, inclusive a 380px |
