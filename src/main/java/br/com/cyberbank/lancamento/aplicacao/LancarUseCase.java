...
public class LancarUseCase {
    ...
    public Lancamento executar(Long ambienteId, Long autorId, Long meioId, Long categoriaId,
        ...
        if (meio.temFatura()) {
            ...
            aLancar = Lancamento.deCompraNoCredito(ambienteId, conta.id(), meio.id(), categoriaId,
                    autorId, sentido, valorCentavos, dataEvento, descricao, aberta.id(),
                    estabelecimento, relogio.instant());
        } else {
            ...
            aLancar = Lancamento.doUsuario(ambienteId, conta.id(), meio.id(), categoriaId, autorId,
                    sentido, valorCentavos, dataEvento, dataEfeito, descricao, situacao, null,
                    estabelecimento, relogio.instant());
        }

        Lancamento lancado = lancamentos.salvar(aLancar);
