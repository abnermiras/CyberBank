...
public class LancarUseCase {
    ...
    public LancarUseCase(LancamentoRepository lancamentos, ContaRepository contas,
            MeioRepository meios, EscolhaDeCategoria escolhaDeCategoria, DiaLocal diaLocal,
            Clock relogio) {
    ...
    public Lancamento executar(Long ambienteId, Long autorId, Long meioId, Long categoriaId,
        ...
        Situacao situacao = dataEfeito.isAfter(diaLocal.hoje())
                ? Situacao.PREVISTO
                : Situacao.REALIZADO;

        return lancamentos.salvar(Lancamento.doUsuario(ambienteId, conta.id(), meio.id(),
                categoriaId, autorId, sentido, valorCentavos, dataEvento, dataEfeito, descricao,
                situacao, estabelecimento, relogio.instant()));
    }
}
