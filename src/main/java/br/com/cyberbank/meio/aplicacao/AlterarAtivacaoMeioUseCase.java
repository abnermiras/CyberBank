...
public class AlterarAtivacaoMeioUseCase {
    ...
    public AlterarAtivacaoMeioUseCase(MeioRepository meios) {
        this.meios = meios;
    }

    @Transactional
    public Meio executar(Long ambienteId, Long meioId, boolean inativo) {
        Meio meio = meios.buscarDoAmbiente(meioId, ambienteId)
                .orElseThrow(() -> new RegraDeDominioException(CodigoDeErro.NAO_ENCONTRADO));
        return meios.salvar(meio.comAtivacao(inativo));
    }
}
