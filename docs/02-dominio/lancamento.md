...
| **`parcelamento` e `recorrencia`** | Os dois campos não existem nem na tabela. Sem `parcelamento` não há compra dividida, e o eixo *por compra* do relatório não tem o que separar; `recorrencia` é Fase 2. **`fatura` existe desde o cartão**, e com ele o `PROVISIONADO` |
